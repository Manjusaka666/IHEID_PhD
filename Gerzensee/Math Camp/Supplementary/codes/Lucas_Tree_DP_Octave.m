...
%rng("default");     %Initialize the random number generator
                     %For Octave rng("default") instead of rng('default')
epsilon=normrnd(0,1,1,500);
rng("default");      %Initialize the random number generator
                     %For Octave rng("default") instead of rng('default')
r_ln=lognrnd(0,sigma,1,500);
...
for ell=1:max_iter
  for i=1:dim_y
    f_new(i) = h(i) +  beta* mean( interp1(y_vec,f,(y_vec(i).^alpha).*r_ln,'spline', 'extrap') );
    %For Octave, 'extrap' added (otherwise interp1 may return NaN)
...
if latex_interpreter==1
  ...
  title('Equlibrium Pricing Function p','FontSize',14);
end
