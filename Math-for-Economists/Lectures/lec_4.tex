...
\begin{definition}[Interior, closure, boundary]\label{def:interior-closure}
	...
	\begin{enumerate}[(i)]
		\item A point $a\in X$ is \dfnas{interior to}{interior point} $A$ if
		      $\ball{\varepsilon}{a}\subseteq A$ for some $\varepsilon>0$. The
		      \dfn{interior} $\Int A$ is the set of interior points of $A$; by
		      definition $\Int A\subseteq A$. The set $A$ is \dfnas{open}{open set} if
		      $A=\Int A$.
		\item A point $a\in X$ is a \dfnas{point of closure}{point of closure} of $A$
		      ...
		      definition $A\subseteq\closure{A}$. The set $A$ is \dfnas{closed}{closed
			      set} if $A=\closure{A}$.
...
\autocite[slide 35]{fukuda2026analysis}. It is worth stating explicitly because
almost every proof about closed sets is obtained by complementing a proof about
open sets.
...
\begin{definition}[Topological space]\label{def:topological-space}
	...
	\dfnas{metrisable}{metrisable topology}.
\end{definition}
...
\begin{definition}[Sequence, subsequence, limit]\label{def:sequence}
	...
	The sequence \dfnas{converges}{convergent sequence} to $x^\ast\in X$, written
...
\begin{definition}[Compactness]\label{def:compact}
	...
	\begin{enumerate}[(i)]
		...
		\item $K$ is \dfnas{sequentially compact}{sequentially compact set} if every
...
\begin{eg}[The budget set is compact]\label{eg:budget-compact}
	...
	The existence half of \autoref{eg:consumer-setup} rests on exactly this
...
\begin{proof}
	If $A$ is not an interval, take $x<z<y$ with $x,y\in A$, $z\notin A$; then
	$A\cap(-\infty,z)$ and $A\cap(z,\infty)$ are disjoint, non-empty, relatively
	open, and cover $A$, so $A$ is not connected. The converse, and the convexity
	statement, follow from the observation that a continuous image of a connected
	set is connected (\autoref{prop:continuous-connected}) applied to the segment
	map $t\mapsto(1-t)x+ty$, whose domain $[0,1]$ is connected.
\end{proof}
