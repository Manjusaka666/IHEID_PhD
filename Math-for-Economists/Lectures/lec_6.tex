...
\begin{definition}[Completeness]\label{def:complete-space}
	A metric space $(X,d)$ is \dfnas{complete}{complete metric space} if every
...
The chain of implications is worth making explicit, because it locates
completeness within the logical structure of the real numbers: the least upper
...
\begin{remark}[Supremum-norm convergence is uniform convergence]
	...
	programming precisely because convergence in it is strong enough to carry
...
The three-term split is the standard device for transferring a property from the
approximants to the limit, and each of the three terms is controlled by a
different hypothesis: the outer two by uniformity of the convergence, the middle
one by continuity of $f_k$.
...
\begin{definition}[Contraction]\label{def:contraction}
	...
	\dfnas{contraction}{contraction mapping} with \dfn{modulus} $\beta$ if there is
	$\beta\in[0,1)$ with
	\[
		d(Tx,Ty)\leq\beta\,d(x,y)\qquad\text{for all }x,y\in X .
	\]
	A point $x^\ast\in X$ with $Tx^\ast=x^\ast$ is a \dfnas{fixed point}{fixed
		point} of $T$.
...
\begin{remark}[How to prove that a value function is concave]\label{rem:property-P}
	...
	closedness requirement is exactly why the supremum norm is the right one:
...
\begin{theorem}[Blackwell]\label{thm:blackwell}
	...
	satisfy
	\begin{enumerate}[(i)]
		\item \dfnas{monotonicity}{monotonicity (Blackwell)}: $f\leq g$ implies
		      $Tf\leq Tg$; and
		\item \dfnas{discounting}{discounting (Blackwell)}: there is
...
\paragraph{What this establishes and what it does not.} It establishes that the
...
\paragraph{Economic content.} Three readings. First, uncertainty neither vanishes
