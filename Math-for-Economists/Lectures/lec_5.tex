...
and lower semicontinuity for minima. Indirect utility functions, value functions of dynamic programs and profit functions
...
\begin{definition}[Relative topology]\label{def:relative-topology}
	...
	\dfnas{relatively open set}{open relative to $A$} if $U=G\cap A$ for some open
	...
	analogously. This is the topology induced by restricting $d$ to
...
\begin{definition}[Continuity]\label{def:continuity}
	...
	\dfnas{continuity}{continuous at} $x_0\in A$ if for every $\varepsilon>0$ there
	...
	\]
	It is \dfnas{continuous mapping}{continuous on $A$} if it is continuous at every
	point of $A$.
\end{definition}
...
restriction to \emph{finitely} many functions is essential and is what
...
\begin{definition}[Uniform, Lipschitz and H\"older continuity]\label{def:uniform-continuity}
	...
	\begin{enumerate}[(i)]
		\item $f$ is \dfnas{uniform continuity}{uniformly continuous} on $A$ if for
		...
		\item $f$ is \dfnas{Lipschitz continuity}{Lipschitz continuous} with constant
		      $L\geq0$ if $d(f(x),f(y))\leq L\,d(x,y)$ for all $x,y\in A$. If
		      $L<1$ the map is a contraction (\autoref{def:contraction}).
		\item $f$ is \dfnas{Holder continuity@H\"older continuity}{H\"older continuous}
...
isolates what is needed, and it is the form in which the hypothesis is
...
\begin{definition}[Semicontinuity]\label{def:semicontinuity}
	...
	\begin{enumerate}[(i)]
		\item $f$ is \dfnas{lower semicontinuity}{lower semicontinuous} (lsc) if
		...
		\item $f$ is \dfnas{upper semicontinuity}{upper semicontinuous} (usc) if $-f$
...
This is Proposition~8.10 of \textcite[p.~271]{fukuda2026notes}. Verbally: at a
point of lower semicontinuity the function
...
at the top of a jump. \autoref{fig:semicontinuity} draws the two cases.

\begin{figure}[htbp]
	\centering
	\input{Figures/semicontinuity.tex}
	\caption[Upper and lower semicontinuity at a jump]{%
		The two ways a jump can be closed, for a function equal to $a$ on one side
		of $x_0$ and to $b>a$ on the other. In (a) the value at $x_0$ is $b$, so
		$\limsup_{x\to x_0}f(x)=b=f(x_0)$ and the function is upper semicontinuous
		at $x_0$; it is not lower semicontinuous, since
		$\liminf_{x\to x_0}f(x)=a<b$. In (b) the value at $x_0$ is $a$ and the two
		conclusions are exchanged. Filled circles mark the value taken at $x_0$.}
	\label{fig:semicontinuity}
\end{figure}
...
\begin{proof}
	Suppose $f$ is lsc and $(x_k,z_k)\to(x,z)$ in $A\times\R$ with $f(x_k)\leq z_k$.
	\autoref{prop:semicontinuity-pointwise} gives
	$f(x)\leq\liminf_kf(x_k)\leq\liminf_kz_k=z$, so that $(x,z)\in\epi f$; now
	apply \autoref{thm:closed-sequential}.

	Conversely, let $\epi f$ be closed and $x_k\to x$ in $A$; we must show
	$f(x)\leq\ell$, where $\ell\eqdef\liminf_kf(x_k)\in[-\infty,+\infty]$. If
	$\ell=+\infty$ there is nothing to prove. If $\ell=-\infty$, choose a
	subsequence with $f(x_{k_j})\to-\infty$; then for each fixed $z\in\R$ one has
	$f(x_{k_j})\leq z$ for all large $j$, so $(x_{k_j},z)\in\epi f$, and
	$(x_{k_j},z)\to(x,z)$ forces $(x,z)\in\epi f$, that is $f(x)\leq z$. As
	$z\in\R$ was arbitrary this contradicts $f(x)\in\R$, so the case
	$\ell=-\infty$ does not arise. There remains $\ell\in\R$: pass to a
	subsequence along which $f(x_{k_j})\to\ell$, so that
	$(x_{k_j},f(x_{k_j}))\in\epi f$ converges to $(x,\ell)\in A\times\R$; closedness
	gives $(x,\ell)\in\epi f$, that is $f(x)\leq\ell$. The hypograph statement
	follows by applying this to $-f$.
\end{proof}
...
\begin{eg}[A supremum of continuous functions that is not usc]\label{eg:sup-not-usc}
	...
	continuity of the objective, which is the content of Berge's theorem in
...
\begin{proof}
	...
	A second proof avoids semicontinuity altogether and generalises differently. By \autoref{prop:continuous-compact} the image $f(A)$
...
\begin{eg}[Each hypothesis is needed]\label{eg:weierstrass-failures}
	\begin{enumerate}[(i)]
		...
		\item \emph{Infinite dimensions.} Let $B$ be the closed unit ball of the
		      space $\ell^2$ of \autoref{eg:no-bw-infinite} and define
		      $f\colon B\to\R$ by
		      $f(x)=\sum_{k\geq1}\bigl(1-\tfrac1k\bigr)x_k^2$. The series converges
		      because $\sum_kx_k^2\leq1$, and $f$ is continuous on $B$: for
		      $x,y\in B$,
		      $\abs{f(x)-f(y)}\leq\sum_k\abs{x_k-y_k}\abs{x_k+y_k}
			      \leq\norm{x-y}\norm{x+y}\leq2\norm{x-y}$ by Cauchy--Schwarz. Its
		      supremum is $1$, since $f(e^k)=1-1/k$, and the supremum is not attained:
		      $f(x)=1$ would force $\sum_kx_k^2/k\leq\sum_kx_k^2-f(x)\leq0$, hence
		      $x=0$ and $f(x)=0$. The domain is closed and bounded but, by
		      \autoref{eg:no-bw-infinite}, not compact, and
		      \autoref{thm:weierstrass} does not apply. No \emph{linear} functional
		      would serve as the example: every continuous linear functional on
		      $\ell^2$ is $x\mapsto\ip{a}{x}$ for some $a\in\ell^2$
		      (\autoref{thm:riesz-rn} is the finite-dimensional case) and attains its
		      supremum over $B$ at $a/\norm{a}$.
	\end{enumerate}
\end{eg}
...
\paragraph{Economic content of the hypotheses.} Strict positivity of every
...
\paragraph{Uniqueness, interiority and continuity.}
...
both upper and lower hemicontinuous. Both hold on $\R^{n}_{++}\times\R_{+}$,
zero wealth included, by \autoref{prop:budget-continuous} and
\autoref{eg:budget-failures}; what breaks the argument is a price falling to
zero, and what breaks lower hemicontinuity is the absence of a point strictly
inside the budget set.

\subsection{Preferences without a utility function}\label{subsec:cont-preferences}

Preferences, not utility functions, are the primitive. Three statements about
them are often run together, and they are distinct.

Let $\succsim$ be a complete and transitive preference relation on a set
$X\subseteq\R^n$. Say that $\succsim$ has \dfnas{upper contour set}{closed upper contour
	sets} if $U(x)\eqdef\{y\in X : y\succsim x\}$ is closed
relative to $X$ for every $x\in X$. This is a property of $\succsim$ alone. A
function $u\colon X\to\R$ \dfn{represents} $\succsim$ if $u(y)\geq u(x)$ exactly
when $y\succsim x$.

\begin{proposition}[Semicontinuity of a representation and of the relation]
	\label{prop:usc-preferences}
	Let $u\colon X\to\R$ represent $\succsim$.
	\begin{enumerate}[(i)]
		\item If $u$ is upper semicontinuous, then $\succsim$ has closed upper
		      contour sets.
		\item The converse fails: $\succsim$ may have closed upper contour sets while
		      $u$ is not upper semicontinuous.
		\item If $u$ is upper semicontinuous and $\varphi\colon I\to\R$ is
		      \emph{strictly} increasing and continuous on an interval
		      $I\supseteq u(X)$, then $\varphi\circ u$ is upper semicontinuous and
		      represents $\succsim$. Weak monotonicity is not enough: a constant
		      $\varphi$ merges all utility levels and represents the indifference
		      relation instead.
	\end{enumerate}
\end{proposition}

\begin{proof}
	(i) $U(x)=\{y : u(y)\geq u(x)\}$, an upper section of $u$, closed by
	\autoref{def:semicontinuity}(ii).

	(ii) Take $X=[0,1]$ and let $\succsim$ be the usual order, represented by the
	continuous function $u(x)=x$, so that $U(x)=[x,1]$ is closed for every $x$. Let
	$\varphi(t)=t$ for $t\leq\tfrac12$ and $\varphi(t)=t+1$ for $t>\tfrac12$, which
	is increasing on $[0,1]$, and put $\tilde u\eqdef\varphi\circ u$. Then
	$\tilde u$ represents the same $\succsim$, but
	$\{\tilde u\geq1\}=(\tfrac12,1]$ is not closed, so $\tilde u$ is not upper
	semicontinuous.

	(iii) That $\varphi\circ u$ represents $\succsim$ is immediate from strict
	monotonicity of $\varphi$. For upper semicontinuity, let $x_k\to x_0$ in $X$
	and fix $\varepsilon>0$ with $u(x_0)+\varepsilon\in I$. Upper semicontinuity of
	$u$ gives $u(x_k)<u(x_0)+\varepsilon$ for all large $k$, whence
	$\varphi(u(x_k))\leq\varphi(u(x_0)+\varepsilon)$ and
	$\limsup_k\varphi(u(x_k))\leq\varphi(u(x_0)+\varepsilon)$. Letting
	$\varepsilon\downarrow0$ and using continuity of $\varphi$ at $u(x_0)$ gives
	$\limsup_k\varphi(u(x_k))\leq\varphi(u(x_0))$. If instead $u(x_0)$ is the right
	endpoint of $I$, then $u(x_k)\leq u(x_0)$ for every $k$ and the same conclusion
	is immediate. Apply
	\autoref{prop:semicontinuity-pointwise}.
\end{proof}

The distinction matters because upper semicontinuity of a \emph{particular}
representation is not an ordinal property, whereas closedness of the upper
contour sets is. The gap does not, however, affect the existence question, which
can be settled for $\succsim$ directly.

\begin{proposition}[Existence of a greatest element]\label{prop:greatest-element}
	Let $K\subseteq\R^n$ be non-empty and compact and let $\succsim$ be a complete
	and transitive relation on $K$ with closed upper contour sets. Then the set
	\[
		G\eqdef\{x^\ast\in K : x^\ast\succsim y\ \text{for all }y\in K\}
	\]
	of $\succsim$-greatest elements of $K$ is non-empty and compact.
\end{proposition}

\begin{proof}
	For $y\in K$ the set $U(y)=\{x\in K : x\succsim y\}$ is closed in $K$, hence
	compact by \autoref{prop:compact-stability}(i), and non-empty because
	$y\in U(y)$ by completeness. Given finitely many $y_1,\dots,y_m\in K$,
	completeness and transitivity produce an index $j$ with $y_j\succsim y_i$ for
	all $i$ --- take a maximal element of a finite set under a complete transitive
	relation, obtained by induction on $m$ --- and then
	$y_j\in\bigcap_{i\leq m}U(y_i)$, so every finite subfamily of $(U(y))_{y\in K}$
	has non-empty intersection. \autoref{lem:finite-intersection} gives
	$G=\bigcap_{y\in K}U(y)\neq\emptyset$, and $G$ is an intersection of closed
	subsets of the compact set $K$, hence compact.
\end{proof}

\autoref{prop:demand-exists} is the special case $K=\budget(p,m)$ with
$\succsim$ represented by an upper semicontinuous $u$; the argument above shows
that the utility function is a convenience rather than a necessity, and that the
substantive hypotheses are compactness of the feasible set and closedness of the
upper contour sets. What a utility representation buys is the machinery of
\autoref{ch:calculusn} and \autoref{ch:optimization}, which needs a real-valued
