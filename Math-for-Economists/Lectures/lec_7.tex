...
\begin{definition}[Derivative]\label{def:derivative}
	...
	$x_0\in(a,b)$. The function $f$ is \dfnas{differentiability}{differentiable at}
	$x_0$ if
	\[
...
\begin{definition}[Concave and convex functions of one variable]\label{def:concave-1d}
	...
	\dfnas{concave function}{concave} if
	\[
	...
	$\lambda\in(0,1)$, and \dfnas{convex function}{convex} (respectively strictly
...
parameter goes to zero --- which is the situation in
...
\begin{eg}[Log-linearisation]\label{eg:log-linearization}
	...
	step by which a non-linear dynamic model is converted into a linear system in log
...
First, \eqref{eq:lerner} is the Lerner condition, usually
...
direction --- so \autoref{prop:inverse-derivative} carries the derivation. Third, profit maximisation requires $\eta<-1$: if demand were
...
\begin{theorem}[Arrow--Pratt]\label{thm:arrow-pratt}
	...
	\]
	is the \dfnas{absolute risk aversion}{coefficient of absolute risk aversion}.
...
\begin{proof}
	...
	\emph{The order of $\pi$.} Differentiating the identity $\Phi(\pi(t),t)=0$ and
	using \eqref{eq:ift-derivative},
	\[
		\pi'(t)=-\frac{\partial\Phi/\partial t}{\partial\Phi/\partial\pi}
		=-\frac{\E\bigl[u'(w+t\tilde z)\,\tilde z\bigr]}{u'\bigl(w-\pi(t)\bigr)} .
	\]
	At $t=0$ the numerator is $u'(w)\E[\tilde z]=0$, so $\pi'(0)=0$. Since $\pi$ is
	twice continuously differentiable with $\pi(0)=\pi'(0)=0$,
	\autoref{thm:taylor-1d} gives $\pi(t)=O(t^2)$ as $t\downarrow0$. This is what
	licenses the truncation below; it is a conclusion, not an assumption.
	...
	$u(w-\pi)=u(w)-u'(w)\pi+O(\pi^2)$. Equating and using the bound
	$\pi=O(t^2)$ established above, which makes $O(\pi^2)=O(t^4)$,
	\[
