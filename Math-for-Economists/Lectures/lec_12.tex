...
% (2004), Mangasarian (1994), Milgrom-Segal (2002).
...
\begin{definition}[The programming problem]\label{def:program}
	Let $f,g_1,\dots,g_n\colon\R^m\to\R$. The \dfnas{programming
		problem}{programming problem}, henceforth \dfnas{Problem 1}{Problem 1}, is
	\begin{equation}\label{eq:problem-1}
		\max_{x\in\R^m}\ f(x)
		\qquad\st\quad g_j(x)\geq0\ \text{ for each }j\in\{1,\dots,n\}.
	\end{equation}
	The inequalities are the \dfnas{constraints}{constraint}, $f$ is the
	\dfn{objective function}, and
	\[
		D\eqdef\{x\in\R^m : g_j(x)\geq0\ \text{for each }j\}
	\]
	is the \dfnas{domain}{domain of a program} of the program. The program is
	\dfnas{bounded}{bounded program} if $f$ is bounded above on $D$.
\end{definition}
...
\begin{definition}[Binding and slack]\label{def:binding}
	The constraint $g_j(x)\geq0$ is \dfnas{binding}{binding constraint} at
	$x^\ast\in D$ if $g_j(x^\ast)=0$, and \dfnas{slack}{slack constraint} if
	$g_j(x^\ast)>0$.
\end{definition}
...
problems have the same solutions under suitable hypotheses. Stating them
...
\begin{definition}[Problem 5: saddle point]\label{def:problem-5}
	...
	\end{equation}
	Find $(\bar x,\bar y)\in\R^m\times\R^n_+$ that is a \dfnas{saddle
		point}{saddle point} of $F$:
...
\begin{proof}
	...
	\]
	has no solution. \autoref{ass:slater} is exactly the solvability hypothesis
...
\begin{remark}[Constraint qualifications]\label{rem:constraint-qualification}
	...
	they are collectively the \dfnas{constraint qualifications}{constraint
		qualification}. Three points about them.
	...
	nonlinear constraints such as incentive compatibility.
...
\begin{eg}[Failure of the constraint qualification]\label{eg:cq-failure}
	Maximise $f(x_1,x_2)=x_1$ subject to
	$g_1(x)=-x_2\geq0$ and $g_2(x)=x_2-(1-x_1)^3\geq0$. The feasible set is
	$\{(x_1,0) : x_1\geq1\}$ together with the region below the cubic, and
	$\bar x=(1,0)$ is a local maximiser on the curve. At $\bar x$ both constraints
	bind, and $\nabla g_1(\bar x)=(0,-1)^{\transp}$,
	$\nabla g_2(\bar x)=(0,1)^{\transp}$ are linearly dependent. No
	$y\in\R^2_+$ satisfies $\nabla f(\bar x)+y_1\nabla g_1+y_2\nabla g_2=0$, since
	the first component of $\nabla f$ is $1$ and both constraint gradients have
	first component $0$. The first-order conditions therefore have no solution at a
	point that is optimal, and a search over stationary points would miss it.
\end{eg}
...
\begin{remark}[Differentiability of $V$ is an assumption, not a conclusion]
	...
	conditions, and requires the bordered Hessian to be nonsingular at the optimum.
...
\path{consumer_choice_sqp_Octave.m} and \path{consumer_choice_python.py}. Two observations
are worth making about the comparison. The solver returns a multiplier alongside
the solution, and it is the same $y_1^\ast$; a numerical multiplier that differs
from $\partial V/\partial m$ computed by finite differences signals either a
non-binding constraint or a failure of the constraint qualification. And the
solver requires the problem in minimisation form with constraints written as
$c(x)\leq0$, which is the sign flip of \autoref{def:program}: the multiplier
returned is then non-negative for the reversed convention, and reading its sign
without checking the convention is the most common error in applied work.
...
tomorrow, scaled by the gross return. Three comparative statics follow, each
requiring a different piece of the machinery. The consumption growth rate
...
ambiguous in sign and is positive exactly when $\gamma>1$, the income effect
dominating the substitution effect. And $\lambda^\ast=u'(c_1^\ast)$ is the
marginal utility of wealth, so by \autoref{thm:envelope}
$\partial V/\partial W=u'(c_1^\ast)$ --- an identity that is trivial here and is
the Benveniste--Scheinkman formula of \autoref{ch:dynamic} in the infinite-horizon
case. The corresponding programs are \path{two_periods_consumption_fmincon.m},
\path{two_periods_consumption_sqp_Octave.m} and
\path{two_periods_consumption_python.py}
\autocite[slides 37--40]{fukuda2026optimization}.
...
The identification with the Cobb--Douglas consumer problem is not a coincidence:
maximising a product subject to a linear constraint is the same problem in both
cases, and the equal split of the Nash solution corresponds to the equal
expenditure shares of symmetric Cobb--Douglas preferences. The numerical
counterparts are \path{Nash_bargaining_fmincon.m},
\path{Nash_bargaining_sqp_Octave.m} and \path{Nash_bargaining_python.py}
\autocite[slides 41--44]{fukuda2026optimization}. Two practical points appear
there. Solving \eqref{eq:nash-bargaining} directly is numerically worse
conditioned than solving the logarithmic version, because the product objective
is flat near the axes; and a solver started at the disagreement point returns it,
since the gradient of the product vanishes there, so the initial guess must lie
in the interior.
