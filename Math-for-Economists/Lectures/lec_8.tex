...
\begin{definition}[Partial derivative and gradient]\label{def:partial}
	...
	$i\in\{1,\dots,n\}$ the \dfnas{partial derivative}{$i$th partial derivative} of
	$f$ at $x$ is
	\[
...
\begin{definition}[Differentiability and the total derivative]\label{def:total-derivative}
	...
	\dfnas{differentiability!in several variables}{differentiable at} $x_0\in A$ if
...
Each implication in \autoref{prop:differentiable-implies} is strictly one-way.
The next three examples exhibit, in turn, partial derivatives without continuity,
continuity and partial derivatives without directional derivatives in the
remaining directions, and all directional derivatives without differentiability.
...
\begin{eg}[Continuity and partials without differentiability]\label{eg:no-differentiability}
	...
	\autocite[Rem.~11.3, p.~374]{fukuda2026notes}.

	Here the failure is already visible in the directional derivatives. For a
	direction $v=(a,b)$,
	\[
		\frac{f(tv)-f(0)}{t}=\frac{t^2ab}{\abs{t}\sqrt{a^2+b^2}}\cdot\frac1t
		=\sgn(t)\,\frac{ab}{\sqrt{a^2+b^2}},
	\]
	so the two-sided limit as $t\to0$ exists only when $ab=0$: apart from the two
	coordinate axes, this $f$ has no directional derivative at all.
\end{eg}

\begin{eg}[All directional derivatives without differentiability]
	\label{eg:directional-not-linear}
	The stronger failure --- every directional derivative exists, yet $f$ is not
	differentiable --- needs a different function. Let
	\[
		f(x,y)=
		\begin{cases}
		...
		\end{cases}
	\]
	Since $\abs{f(x,y)}\leq\abs{x}$, the function is continuous at the origin. For
	$v=(a,b)\neq0$ and $t\neq0$,
	\[
		\frac{f(tv)-f(0)}{t}
		=\frac1t\cdot\frac{t^3a^3}{t^2(a^2+b^2)}=\frac{a^3}{a^2+b^2},
	\]
	which does not depend on $t$; the directional derivative
	$D_vf(0,0)=a^3/(a^2+b^2)$ therefore exists for every $v$. In particular
	$\partial f/\partial x(0,0)=1$ and $\partial f/\partial y(0,0)=0$, so the only
	candidate for a gradient is $(1,0)$. But $v\mapsto D_vf(0,0)$ is not linear:
	with $v=(1,0)$ and $w=(0,1)$ one has $D_{v+w}f(0,0)=\tfrac12$ while
	$D_vf(0,0)+D_wf(0,0)=1$. By \autoref{prop:differentiable-implies} the map
	$v\mapsto D_vf(0,0)$ would have to equal $\ip{\nabla f(0,0)}{\cdot}$ and hence
	be linear, so $f$ is not differentiable at the origin.

	The two examples separate two distinct obstructions. In
	\autoref{eg:no-differentiability} the one-dimensional restrictions of $f$ are
	themselves badly behaved; here each restriction to a line is differentiable and
	the failure is only that the derivatives along different lines do not assemble
	into a linear functional. Both are ruled out by
	\autoref{prop:c1-differentiable}, and the economic reading is the same: a
	comparative-statics exercise conducted one variable at a time can be internally
	consistent along every ray and still fail to describe the response to a joint
	perturbation.
\end{eg}
...
the ratio $\bigl(\partial u/\partial x_i\bigr)\big/\bigl(\partial u/\partial
	x_j\bigr)$ because the level set is orthogonal to the gradient. This is
...
\begin{definition}[Homogeneity]\label{def:homogeneous}
	...
	$\lambda>0$. A function $f\colon X\to\R$ is \dfnas{homogeneous function}{homogeneous of degree
		$k$} if $f(\lambda x)=\lambda^kf(x)$ for all $x\in X$ and
	$\lambda>0$.
\end{definition}
...
\textcite[Thm.~9.41]{rudin1976principles}. Symmetry of the Hessian is what
produces the Slutsky symmetry conditions and the reciprocity relations that make
the Hessian of an expenditure function testable.
...
\begin{theorem}[Implicit function theorem]\label{thm:implicit-function}
	...
	$\nabla_yf(\bar x,\bar y)$ is non-singular. Then there are an open neighbourhood
	...
	\begin{enumerate}[(i)]
		...
		\item $e(\bar x)=\bar y$;
		\item $\nabla_yf\bigl(x,e(x)\bigr)$ is non-singular for all $x\in\Gamma$; and
...
long and purely technical, and because it is the use of the contraction
...
\]
and solving, which is legitimate because (iii) guarantees the inverse
...
\begin{corollary}[Inverse function theorem]\label{thm:inverse-function}
	...
	and $\bar x\in A$ with $Dg(\bar x)$ non-singular. Then there are open
	neighbourhoods $U\ni\bar x$ and $V\ni g(\bar x)$ such that
...
\begin{proof}
	...
	$\nabla_xf=Dg(\bar x)$ is non-singular by hypothesis. The implicit function
...
$g(x_1,x_2)=(e^{x_1}\cos x_2,e^{x_1}\sin x_2)$ has everywhere non-singular
...
\begin{remark}[Comparative statics is the implicit function theorem]
	...
	theorem answers three separate questions.
	...
	The condition that can fail is non-singularity of $\nabla_yf$. What follows from
	its failure is only that the theorem is silent: the hypotheses no longer
	deliver a locally unique solution branch, and \eqref{eq:ift-derivative} has no
	meaning because the matrix to be inverted is singular. It does \emph{not}
	follow that the equilibrium is non-unique, or that comparative statics fail to
	exist. Take $n=m=1$ and $f(x,y)=y^3-x$ at $(\bar x,\bar y)=(0,0)$: here
	$\partial f/\partial y=3y^2$ vanishes at the origin, yet $f(x,y)=0$ has the
	unique global solution $y=x^{1/3}$, which is continuous everywhere and
	differentiable away from $0$. The correct reading is that a singular
	$\nabla_yf$ marks the points at which the implicit function theorem stops
	certifying anything, so that uniqueness, smoothness and the sign of the
	response must be established by other means --- and it is among such points,
	not at every such point, that multiplicity and bifurcation can occur.
\end{remark}
...
\paragraph{Scope of the calculus route.} The calculus route delivers existence,
uniqueness and a formula, and it makes the role of the data condition explicit:
$\sum_i(x_i-\bar x)^2>0$ is simultaneously the non-degeneracy condition on the
...
discontinuous there (\autoref{eg:partials-no-continuity}), be continuous with all
partials and still lack directional derivatives off the axes
(\autoref{eg:no-differentiability}), or possess every directional derivative and
still fail to be differentiable because those derivatives are not linear in the
direction (\autoref{eg:directional-not-linear}). Continuity of the partials is
sufficient
...
\eqref{eq:ift-derivative}; it is the theorem underlying differentiable
comparative statics, and where its hypothesis --- non-singularity of the Jacobian
in the endogenous variables --- fails, the theorem certifies nothing, though a
well-behaved solution branch may still exist (\autoref{rem:comparative-statics-ift}).
...
\begin{exercise}\label{ex:cn-1}
	Let $f\colon\R^2\to\R$ take the value $1$ on the region
	$\{(x,y) : 0<y<x^2\}$ and the value $0$ elsewhere. Show that for every
	$v\in\R^2$ the directional derivative $D_vf(0,0)$ exists and equals $0$, so
	that $v\mapsto D_vf(0,0)$ is linear, and yet $f$ is not continuous at the
	origin. (Consider the curve $t\mapsto(t,t^2/2)$.) Conclude that neither the
	existence of all directional derivatives nor their linearity in $v$ implies
	differentiability, and locate the step of
	\autoref{prop:differentiable-implies} that this example does not contradict.
\end{exercise}
