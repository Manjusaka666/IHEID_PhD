...
This chapter fixes set-theoretic language
...
\begin{definition}[Operations on sets]\label{def:set-ops}
	...
	\begin{enumerate}[(i)]
		...
		\item The \dfn{power set} of $X$ is $\pow{X}\eqdef\{A : A\subseteq X\}$.
		      \nidx{powerset}{$\pow{X}$, power set of $X$}
	\end{enumerate}
\end{definition}
...
\begin{definition}[Mapping]\label{def:mapping}
	...
	\dfnas{range of a mapping}{range}.
\end{definition}
...
\begin{definition}[Injection, surjection, bijection]\label{def:inj-surj}
	...
	\dfnas{injective mapping}{injective} if $f(a)=f(a')$ implies $a=a'$;
	\dfnas{surjective mapping}{surjective} if $f(A)=B$; and
	\dfnas{bijective mapping}{bijective} if it is both. A bijection $f$ has a
...
\begin{definition}[Finite, countable, uncountable]\label{def:countable}
	...
	$\{1,\dots,n\}\to A$ for some $n\in\N$; \dfnas{countably
		infinite set}{countably infinite} if there is a bijection $\N\to A$; \dfn{countable} if it is finite
...
\begin{remark}[Where countability matters in economics]\label{rem:countability-econ}
	Countability decides what can be measured. In \autoref{ch:probability} a probability
	...
	\autocite[Ch.~1]{legall2022measure}. The Gerzensee treatment makes this
...
\begin{definition}[Binary relation]\label{def:relation}
	...
	\begin{enumerate}[(i)]
		\item \dfnas{reflexive relation}{reflexive} if $xRx$ for all $x\in X$;
		\item \dfnas{complete relation}{complete} if for all $x,y\in X$, $xRy$ or $yRx$
		      (or both);
		\item \dfnas{transitive relation}{transitive} if $xRy$ and $yRz$ imply $xRz$;
		\item \dfnas{symmetric relation}{symmetric} if $xRy$ implies $yRx$;
		\item \dfnas{antisymmetric relation}{antisymmetric} if $xRy$ and $yRx$ imply
		      $x=y$.
	\end{enumerate}
\end{definition}
...
\begin{definition}[Utility representation]\label{def:utility-rep}
	A function $u\colon X\to\R$ \dfnas{utility representation}{represents} the
...
\begin{remark}[Why countability is not innocuous]\label{rem:lexicographic}
	...
	lexicographic preference violates this. The general statement is
...
\begin{proof}
	...
	$s-\varepsilon$ would be an upper bound smaller than $s$, contradicting the
	minimality of $s$ among upper bounds.
...
\begin{remark}[The extended real line]\label{rem:extended-reals}
	...
	that the terms are finite. The convention separates two failures in optimisation: the value of an
