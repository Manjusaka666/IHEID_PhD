...
gradient to zero. The solution is correct, and it conceals the structure that generalises. Writing $\mathbf1=(1,\dots,1)^{\transp}$,
...
\begin{definition}[Orthogonal complement]\label{def:orthogonal-complement}
	...
	\]
	A set $A$ is \dfnas{orthogonal set}{orthogonal} if $\ip{x}{y}=0$ for all
	distinct $x,y\in A$, and \dfnas{orthonormal set}{orthonormal} if in addition
...
but the subspace of finitely supported sequences in $\ell^2$ is dense and proper,
so its orthogonal complement is $\{0\}$ and no decomposition of the stated form
exists.
...
\begin{proof}
	...
	\emph{The norm.} Write $\norm{f}\eqdef\sup\{\abs{f(x)} : \norm{x}\leq1\}$ for
	the operator norm. Cauchy--Schwarz (\autoref{lem:cauchy-schwarz}) gives
	$\abs{f(x)}=\abs{\ip{a}{x}}\leq\norm{a}\norm{x}$, so
	$\norm{f}\leq\norm{a}$. For the reverse inequality: if $a=0$ then $f\equiv0$ and
	both norms vanish; otherwise $x\eqdef a/\norm{a}$ has $\norm{x}=1$ and
	$f(x)=\ip{a}{a}/\norm{a}=\norm{a}$, so $\norm{f}\geq\norm{a}$. Hence
	$\norm{f}=\norm{a}$, and the supremum defining $\norm{f}$ is attained.
\end{proof}
...
\subsection{Gram--Schmidt and QR}\label{subsec:proj-qr}
...
\paragraph{Consequences of the projection reading.} Four things that the calculus derivation
leaves opaque.
...
\begin{eg}[The simulation]\label{eg:ols-simulation}
	...
	\path{regression_Python.py} --- build $X=[\mathbf1\ \ x]$ and evaluate
	\verb|inv(X'*X)*X'*y|, comparing the result with the built-in
	\verb|regress| in \textsc{Matlab} and with \verb|numpy.linalg.lstsq| in Python.
	All three fix a seed --- \verb|rng(0)| in the \textsc{Matlab} and Octave
	versions, \verb|np.random.seed(1)| in the Python one --- but the three
	generators are different implementations, so the draws are not comparable
	across languages and the three realisations of $\hat\beta$ are not meant to
	agree. What agrees within each program is the hand-coded normal-equation
	solution and the library routine. Executed on \textsc{Matlab} R2025b,
	\path{regression.m} returns $\hat\beta=(0.955343,2.940044)^{\transp}$ from
	both routes, agreeing to $1.6\times10^{-15}$; \path{regression_Python.py}
	returns $\hat\beta=(1.023019,3.021874)^{\transp}$ from both. The Octave
	variant, translated line by line into \textsc{Matlab} syntax and run there,
	reproduces the \textsc{Matlab} figures exactly, as its seed and construction
	are identical. Its second \verb|fprintf| is labelled ``\verb|regress|'' but
	prints \verb|beta_hat| rather than \verb|beta_hat_2|, so the comparison it
	advertises is not the one it displays; the two vectors coincide to
	$1.6\times10^{-15}$, so the printed numbers are right for the wrong reason.

	The realisation differs from the true $(1,3)^{\transp}$ by an amount that is
	itself the projection of the error vector $\varepsilon$,
	\[
