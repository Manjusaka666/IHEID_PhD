...
of a review course. Every theorem states the space it lives on and the regularity
it requires. Necessary conditions are distinguished from sufficient ones,
existence from uniqueness, and local statements from global ones. Where a
teaching-level simplification appears in the course slides, the simplification is
retained for its intuitive value and the exact statement is given alongside it,
with the relation between the two made explicit; the Kuhn--Tucker theorem in
\autoref{ch:optimization} and the envelope formula in \autoref{ch:dynamic} are
the clearest instances. Proofs are given in full, except where a result is
quoted, in which case the source is named at the point of use and the reason for
omission is stated.
...
this is the part usually left out --- what goes wrong when a hypothesis fails.
...
Each is developed to the point where the mathematics is doing visible work.

\section*{Sources}

The volume rests on three layers of source material, and the reader should know
which is which.

The first layer is the set of six slide decks of the Gerzensee course
...
no topic is included that the course does not need, and no result stated in the
slides is omitted here.

The second layer is the accompanying reference volume
...
The third layer consists of the mathematical texts that carry each topic beyond
...
is a column vector and $Df(x)=\nabla f(x)^{\transp}$ when $m=1$; this is the one
place where a transpose is easy to lose, and it is stated explicitly whenever it
matters.
