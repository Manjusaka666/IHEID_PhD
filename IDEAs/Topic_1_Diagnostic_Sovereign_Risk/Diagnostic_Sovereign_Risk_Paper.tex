...
\input{output/survey_mapping_numbers.tex}
\IfFileExists{output/long_bond_numbers.tex}{\input{output/long_bond_numbers.tex}}{}
\providecommand{\LongFPResidual}{$5\times10^{-8}$}
\providecommand{\FPResidual}{$5\times10^{-8}$}
\providecommand{\FPInitDiff}{$10^{-6}$}

\title{Diagnostic Expectations and Sovereign Default}
\author{[Author]}
\date{July 2026}
...
\begin{abstract}
\noindent Recent news becomes a pricing state when sovereign creditors form diagnostic expectations. In a one-period debt model, equal debt and equal income command different prices after different histories. Good news raises prices and predicts low bond returns under the true probability law. A monthly measurement model maps individual fixed-event forecast revisions into the diagnostic parameter. A five-year bond implementation preserves an exact rational history zero and quantifies the diagnostic effects on spreads, default, and returns. Capital-constrained rational intermediaries attenuate the news slope continuously as their risk-bearing capacity rises. The quantitative exercise imports the published long-debt calibration of \citet{ChatterjeeEyigungor2012} and reports simulation uncertainty for every principal moment. Debt ceilings raise sovereign welfare when the government extrapolates and lower it when only creditors extrapolate.
\end{abstract}
...
The quantitative work separates the short-bond theorem from the long-bond mechanism. The one-period economy uses the \citet{Arellano2008} calibration. At $\theta=0.5$, one standard deviation of good news lowers the spread by \NewsBpDiagAbs\ basis points at fixed debt and output, while rational pricing gives zero. The long-bond economy uses the complete published parameter block of \citet{ChatterjeeEyigungor2012}. It adds coupons, dilution, and resale prices. At $\theta=0$, history remains an exact redundant state. At $\theta>0$, price monotonicity and the return sign are numerical properties of the computed long-bond equilibrium, reported with fixed-point residuals and simulation uncertainty.
...
\citet{NiemannPrein2025} place diagnostic expectations in a quantitative long-term sovereign debt model, estimate diagnosticity from IMF growth forecasts, and study debt and spread brakes. Those contributions establish the broad quantitative mechanism. I pursue the restrictions that identify diagnostic sovereign pricing. The short-bond theory isolates history dependence at fixed debt and income. The survey block reproduces the monthly timing of individual fixed-event forecasts. The central market test uses future sovereign bond returns. The intermediary extension shows how rational risk-bearing capacity attenuates the news slope. The two belief assignments in the policy exercise separate gains from creditor mispricing from losses caused by government extrapolation. These restrictions distinguish the paper from an additional long-debt calibration of diagnostic expectations.

The remaining comparison is with rational sovereign default models and the wider evidence on belief distortions. Quantitative default models explain countercyclical spreads through default incentives \citep{AguiarGopinath2006, Arellano2008, ChatterjeeEyigungor2012, MendozaYue2012}. Multiplicity models generate history dependence through equilibrium selection \citep{ColeKehoe2000, LorenzoniWerning2019}. Diagnostic beliefs instead give public news a signed return implication. Forecast research measures the same extrapolation outside asset prices \citep{BordaloGennaioliMaShleifer2020}. Combining the forecast measure with future bond returns supplies the empirical discipline.
...
At the start of a period the government owes $b \geq 0$ and chooses whether to repay or default. Under repayment it pays $b$ and sells new debt $b' \in \mathcal{B} \subset \R_+$ at the market price $q$, giving consumption $c = y + q b' - b$. Default writes off the debt, excludes the country from markets, and imposes the output loss standard in this literature: income during exclusion is $y^D(x) = \min\{y, \phi\, \E[y]\}$. This asymmetric cost bites in good states and gives realistic default frequencies in \citet{Arellano2008}. \citet{MendozaYue2012} derive it from imported-input substitution. Each period, exclusion ends with probability $\lambda$ and the country re-enters with zero debt.
...
Diagnostic lenders price the marginal unit of emerging-market debt. Individual professional analysts, the population represented in the model, revise too far in the direction of news \citep{BordaloGennaioliMaShleifer2020, BordaloEtAl2022JEP}. An arbitrageur who trades against this distortion must carry a large position and bear default risk until beliefs correct. Limited intermediary capital therefore lets diagnostic beliefs enter competitive prices \citep{GreenwoodHanson2013, LopezSalidoSteinZakrajsek2017}. Risk-neutral diagnostic pricing supplies the behavioral counterpart of rational risk-neutral pricing in the workhorse model. The baseline keeps the government rational and assigns overreaction to the market, which isolates the pricing distortion. Section~\ref{sec:rules} then completes the two-by-two of government and market beliefs for normative analysis.
...
The proof in Appendix~\ref{app:proofs} applies Kakutani's theorem to prices and mixed stationary policies. The reported deterministic equilibrium satisfies Bellman and price residuals below \FPResidual. Iteration from risk-free prices, zero prices, and the rational price schedule produces price arrays whose largest difference is \FPInitDiff. At $\theta = 0$, $h$ drops out of \eqref{eq:price} and the model is exactly \citet{Arellano2008}.
...
\begin{lemma}[A sufficient condition for income-monotone default]\label{lem:income_sufficient}
Consider the one-period economy with i.i.d.\ income and proportional default consumption $y^D=(1-\psi)y$, where $0<\psi<1$. Fix inherited debt and any belief state. Suppose every feasible repayment plan satisfies $c^R(y,b')\leq \bar c(y)$ and
\[
u'\!\left(\bar c(y)\right)
\geq (1-\psi)u'\!\left((1-\psi)y\right).
\]
...
Consensus Economics surveys professional forecasters on current-year and next-year real GDP growth. The survey records these fixed-event forecasts each month. Let $s_t$ be the number of months remaining in the current year. The fixed-horizon object is
\[
...
The reported long-debt exercise transports the published parameter block and changes only $\theta$. This external calibration keeps maturity, default technology, patience, and income risk tied to one internally coherent source. It also gives the comparison a direct interpretation. Every difference between the two columns of Table~\ref{tab:longmoments} comes from the probability law used by lenders.

The replication code also defines a six-moment SMM estimator for a matched country panel. It takes the target covariance matrix as an input, reports scale-diagonal and covariance-optimal weights, computes the moment Jacobian by branch-preserving continuation, tests its local rank, and forms sandwich intervals for $(\beta,d_0,d_1)$. The available published moments support the transported calibration. A matched country panel and its joint moment covariance activate the SMM stage. The results below use the published parameter block and attach uncertainty to every simulated outcome.
...
\begin{tabular}{lrrrr}
\toprule
 & \multicolumn{2}{c}{$\theta=0$} & \multicolumn{2}{c}{$\theta=0.5$} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
 & Estimate & S.E. & Estimate & S.E. \\
\midrule
Median annual spread, percent & \LongSpreadMedRE & \LongSpreadMedREse & \LongSpreadMedDiag & \LongSpreadMedDiagse \\
Spread s.d., percent & \LongSpreadSdRE & \LongSpreadSdREse & \LongSpreadSdDiag & \LongSpreadSdDiagse \\
Defaults per 100 years & \LongDefRE & \LongDefREse & \LongDefDiag & \LongDefDiagse \\
Mean debt to output, percent & \LongDebtRE & \LongDebtREse & \LongDebtDiag & \LongDebtDiagse \\
Debt service to output, percent & \LongDebtServiceRE & \LongDebtServiceREse & \LongDebtServiceDiag & \LongDebtServiceDiagse \\
Within-cell news coefficient, bp per s.d. & 0.0 & \LongNewsREse & $-\LongNewsBp$ & \LongNewsDiagse \\
Expected excess return after good news, bp & 0.0 & \LongReturnGoodREse & \LongReturnGood & \LongReturnGoodDiagse \\
Expected excess return after bad news, bp & 0.0 & \LongReturnBadREse & \LongReturnBad & \LongReturnBadDiagse \\
\bottomrule
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\textwidth}
\footnotesize Notes: the table holds the published long-debt parameter block fixed across columns. Standard errors use 250 nonoverlapping batches of 2,000 simulated quarters and preserve the model's serial dependence. The rational return entries equal zero up to the reported fixed-point tolerance.
\end{minipage}
\end{table}
}{}

Long debt changes the return object. A lender receives principal, coupon, and the resale price of the surviving claim. The rational history zero remains exact. At $\theta=0.5$, the computed equilibrium produces a negative expected excess return after good news and a positive return after bad news. These long-bond signs are numerical properties rather than corollaries of Proposition~\ref{prop:returns}, since resale prices and future issuance enter the payoff. The one-period result isolates the theorem without dilution.
...
The default frequency rises from \DefFreqRE\ to \DefFreqDiag\ per century, against 2--3 per century in Argentine data. \citet{Arellano2008} selected the parameters outside beliefs to match default frequency at $\theta=0$. Table~\ref{tab:moments} therefore traces the fixed-parameter comparative statics in $\theta$. Section~\ref{sec:recal} re-selects $(\beta,\phi)$ at the baseline $\theta$ and reports the surviving moments. Propositions~\ref{prop:history}--\ref{prop:fragility} continue to govern every parameter vector satisfying Assumption~\ref{ass:monotone}. At $\theta=1$, the economy loses market access a quarter of the time. This stress calculation establishes the value of estimating $\theta$ inside each sovereign market.
...
\subsection{A short-bond fit diagnostic}\label{sec:recal}

A two-parameter exercise measures how the short-bond mechanism responds when default frequency and debt service return to their data values. At $\theta=0.5$, selecting $(\beta,\phi)$ to match three defaults per century and debt service of 5.5 percent gives $\beta=\BetaRecal$ and $\phi=\PhiRecal$. The median spread becomes \SprMedRecal\ percent. Maturity, spread volatility, and returns remain outside this two-target calculation and enter through the long-bond experiment.

At the selected pair, the within-cell news coefficient is $-\NewsBpRecalAbs$ basis points per standard deviation and remains zero under rational pricing at the same parameters. The reversal-quarter hazards are \HazPeakRecalDiag\ percent and \HazPeakRecalRE\ percent. The conditional news mechanism therefore persists when the short-bond default frequency and debt service return to their target values. Section~\ref{sec:longquant} supplies the long-duration comparison.
...
Future bond returns organize the evidence. Diagnostic creditors pay too much after favorable revisions and earn low subsequent returns under the true probability law. The conditional spread slope then ties that return pattern to the model's history state. Asymmetry near the default boundary and cross-country variation in $\theta_i$ sharpen the mechanism. Table~\ref{tab:feasibility} in Appendix~\ref{app:data} records the data sources and coverage.

\medskip
\noindent\textbf{P1: favorable revisions predict low excess returns.} Let $RX_{i,t+h}$ denote the cumulative EMBI total return in excess of a duration-matched United States Treasury return. The primary regression is
\begin{equation}
RX_{i,t+h}=\alpha_i+\tau_t+\beta_R\mathrm{Rev}_{i,t}
+\eta s_{i,t}+\Gamma'X_{i,t}+\varepsilon_{i,t+h}.
\label{eq:returnpanel}
\end{equation}
...
\medskip
\noindent\textbf{P2: news moves spreads conditional on the model state.} The estimating equation, at country $i$ and month $t$,
\begin{equation}
s_{i,t} \,=\, \alpha_{c(i,t)} + \tau_t + \delta\, \mathrm{Rev}_{i,t} + \Gamma' Z_{i,t} + e_{i,t},
\label{eq:spreadpanel}
\end{equation}
regresses the EMBI stripped spread on the consensus forecast revision of twelve-month-ahead real growth. The fixed effect $\alpha_{c(i,t)}$ indexes country-specific cells of debt and output relative to trend. Time effects absorb global conditions. The vector $Z$ contains the forecast level, ratings, commodity terms of trade, and fiscal revisions. This flexible conditioning implements the model state. Inside the maintained rational default model, the conditional news coefficient equals zero. Broader rational models can load on reserves, maturity walls, political risk, and permanent growth news, so the return regression in \eqref{eq:returnpanel} carries the general overreaction test. Equation~\eqref{eq:spreadpanel} tests the sovereign mechanism that links overreaction to the default state.

\medskip
\noindent\textbf{P3: the price response rises near the default boundary.} Within the model, positive and negative news produce responses of $-$\AsymPosDiag\ and $-$\AsymNegDiag\ basis points per standard deviation. Interacting news with debt raises the response where repayment is marginal. The empirical specification adds $\mathrm{Rev}^+$, $\mathrm{Rev}^-$, and $\mathrm{Rev}\times\mathrm{Debt}$ to \eqref{eq:spreadpanel}. The same interaction enters \eqref{eq:returnpanel} with the opposite payoff implication.

\medskip
\noindent\textbf{P4: the measured belief parameter travels across data sets.} The forecast block estimates $\theta_i$ from one cross-fitting fold. A disjoint fold estimates the return and spread responses. The model predicts stronger return reversal and a steeper conditional price response where $\theta_i$ is larger. The hierarchical second stage integrates over the first-stage distribution of $\theta_i$ rather than treating a noisy generated regressor as data.
...
P2: within-cell news coefficient & model-specific 0 & $\NewsBpDiag$ bp per $1\sigma$ & EMBI and forecasts \\
P3: boundary interaction & 0 & stronger near default & EMBI and forecasts \\
P4: response increasing in $\theta_i$ & flat relation & positive relation & cross-fitted country estimates \\
CG regression, individual forecasts & 0 & \CGbetaHalf & published for US\textsuperscript{a} \\
\bottomrule
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\textwidth}
\footnotesize Notes: \textsuperscript{a}\citet{BordaloGennaioliMaShleifer2020} report negative individual-level forecast-error-on-revision coefficients for most United States macroeconomic series. Section~\ref{sec:emdesign} specifies the emerging-market estimation. Data requirements: J.P.~Morgan EMBI Global stripped spreads (Bloomberg or Datastream), Consensus Economics individual forecaster panel, IMF WEO vintages, CBOE VIX, sovereign ratings. Table~\ref{tab:feasibility} records access and replication files.
\end{minipage}
\end{table}

Table~\ref{tab:evidence} collects the designs. The first test establishes overpricing. The next two locate it in the sovereign default mechanism. The last links the market response to a belief parameter measured outside bond prices.
...
\subsection{Proof of Lemma \ref{lem:income_sufficient}}
...
\[
G_{b'}(y)
=u\!\left(c^R(y,b')\right)+\beta C(b')
-u\!\left((1-\psi)y\right)-\beta C^D.
\]
Current repayment consumption increases one for one with income. Therefore
\[
G_{b'}'(y)
=u'\!\left(c^R(y,b')\right)
-(1-\psi)u'\!\left((1-\psi)y\right).
\]
Concavity of utility and $c^R(y,b')\leq \bar c(y)$ imply
\[
u'\!\left(c^R(y,b')\right)
\geq u'\!\left(\bar c(y)\right)
\geq (1-\psi)u'\!\left((1-\psi)y\right).
\]
...
\]
the repayment advantage is nondecreasing as well. Under CRRA utility, substituting $u'(c)=c^{-\gamma}$ gives the stated consumption bound. Under linear utility, the derivative is $1-(1-\psi)=\psi>0$. $\blacksquare$
...
\]
The solver computes each pair at $\theta=0.5$ and simulates 200{,}000 quarters. This short-bond exercise targets default frequency and debt service. The long-duration model retains the published joint calibration described in Section~\ref{sec:longquant}.
...
\paragraph{Welfare.} Consumption equivalents between configurations use $\Delta = (W_1 / W_0)^{1/(1-\gamma)} - 1$ evaluated at zero debt and median income under the true measure. Policy evaluation computes $W$ for diagnostic-government cells as in Appendix~\ref{app:extensions}.
...
\paragraph{Forecasts.} Consensus Economics provides monthly, individual-forecaster surveys of real GDP growth for the current and next calendar year in the major emerging markets from the early-to-mid 1990s. The code converts fixed-event forecasts to a fixed twelve-month horizon by weighting the two events by months remaining in the year: $F^{12m}_t = \tfrac{m}{12} F^{y}_t + (1 - \tfrac{m}{12}) F^{y+1}_t$, with $m$ the months left in year $y$. Month-over-month changes in $F^{12m}$ give revisions. Within-event-year changes give the fixed-target revisions for the Coibion--Gorodnichenko regressions. The IMF World Economic Outlook vintage database supplies the \emph{first-release} growth outturn, so forecast errors use the information set available before later data revisions.
...
\par\smallskip
\begin{minipage}{0.95\textwidth}
\footnotesize Notes: the two licensed panels supply spreads and individual forecasts. The replication archive contains all code, all public data, and exact extraction instructions for the licensed series.
\end{minipage}
\end{table}
...
\begin{proposition}[The rational history zero with long debt]\label{prop:longzero}
Set $\theta=0$ and restrict attention to minimal-state Markov equilibria. The long-bond Bellman and pricing operators preserve the subspace in which values, policies, and prices are independent of $h$. Any fixed point reached in that subspace has $q(b',x,h)=q(b',x)$, so the within-cell news coefficient equals zero for every maturity and coupon. If the unrestricted fixed point is unique, every stationary Markov equilibrium has this property.
\end{proposition}

\noindent\textit{Proof.} At $\theta=0$, the lender kernel depends on $x$ and not on $h$. Suppose the current price and continuation objects are independent of $h$. The sovereign's budget, current utility, and rational continuation value then depend on $(b,x,m)$ alone. Its optimal issuance and default rules inherit the same state space. Substituting these policies into the lender equation makes the payoff and its expectation independent of $h$. The joint operator therefore maps the history-invariant subspace into itself. Iteration from a history-invariant initial condition remains in that closed subspace, and every limit fixed point has the stated price schedule. Uniqueness extends the result to the unrestricted state representation. $\blacksquare$

For $\theta>0$, the resale price and next-period issuance rule both respond to history. The short-bond FOSD proof no longer signs every term in the long-bond payoff. Table~\ref{tab:longmoments} therefore reports long-bond price monotonicity and return signs as computed equilibrium properties.

The continuous shock supplies the computational regularization used by \citet{ChatterjeeEyigungor2012}. For every $(b,x,h)$, the algorithm recovers all thresholds at which the optimal debt choice changes. It integrates bond payoffs exactly over the associated probability masses and integrates utility by twenty-node Gaussian quadrature within each fixed-policy interval. Adaptive quadrature provides an independent accuracy check. The fixed-point objects are $Z$, the integrated exclusion value, and $q$. Values receive full Bellman updates while prices use damping and safeguarded Anderson acceleration. Continuation in $\theta$ begins on a 12-node debt grid. Linear interpolation prolongs the target solution to 50 debt nodes, where the original operator is resolved. The largest continuation, exclusion, or pricing residual is \LongFPResidual. Risk-free and zero price schedules lead to the same low-resolution equilibrium. The one-period limit reproduces the separate short-bond code. Near ties can generate deterministic policy cycles on intermediate grids. The mixed-policy existence result covers these ties, and the reported 50-node deterministic equilibrium satisfies the original fixed-point equations.

The economic parameter block comes from \citet{ChatterjeeEyigungor2012}. It sets $\delta=0.05$, $z=0.03$, $r^*=0.01$, the re-entry probability to $0.0385$, and $\beta=0.95460$. The persistent income process has $\rho=0.948503$ and innovation standard deviation $0.027092$. The default loss is $\max\{0,d_0y+d_1y^2\}$ with $d_0=-0.18845$ and $d_1=0.24559$. That paper selected these values jointly to match mean debt, the mean spread, and spread volatility. Holding the block fixed gives the diagnostic comparison its controlled interpretation.

\subsection{Capital-constrained rational intermediaries}\label{app:intermediaries}
...
At $\theta=1$ the one-period economy defaults \DefFreqOne\ times per century and holds market access \FracAccessOne\ percent of the time. Median spreads fall to \SprMedOne\ percent while the winsorized standard deviation rises sharply. This fixed-parameter stress test shows how sovereign forecast measurement disciplines the transfer of credit-cycle estimates across asset classes. Figure~\ref{fig:rule} reports the full ceiling grids behind Table~\ref{tab:rules}. The optimal ceilings are $\bar b=\CeilOptC$ and $\bar b=\CeilOptD$ in the diagnostic-government cells. Replication code records wall time, iteration counts, residuals, and random seeds for every configuration.
