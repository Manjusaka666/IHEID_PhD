...
\begin{abstract}
\noindent The workhorse model of sovereign default prices debt as a function of current output and the stock of debt: conditional on the state, pricing is memoryless. I replace the lenders' rational expectations with diagnostic expectations, a disciplined model of overreaction to news, in an otherwise standard Eaton--Gersovitz economy. One new state variable appears: the direction of recent income news. The same debt and the same output are priced differently after good news than after bad, and expected bond returns are predictably low after booms. Default regions expand when optimism reverses. The overreaction parameter is identified from individual forecast-revision regressions rather than spreads, and the mapping from the Coibion--Gorodnichenko coefficient to the model is exact. I set $\theta = 0.5$, inside the published individual-forecast range, and specify the emerging-market forecast panel that estimates country-level $\theta_i$. At this value, a boom-reversal quarter carries a default hazard of \HazPeakDiag\ percent against \HazPeakRE\ percent under rational pricing on the identical income path, a gap that survives recalibrating the parameters outside beliefs to restore the observed default frequency. Whether fiscal rules are worth having turns on whose beliefs are distorted: a debt ceiling lowers sovereign welfare when only the market overreacts, and raises it when the government shares the market's beliefs.
\end{abstract}
...
Lemma~\ref{lem:bmono} is unconditional: a larger inherited debt lowers consumption at every issuance choice and leaves the continuation problem unchanged, while the default value is independent of $b$. The proof is three lines in Appendix~\ref{app:proofs}. The income margin is the paper's single maintained regularity condition.
...
Proposition~\ref{prop:fragility} is where the model earns the word \emph{cycle}. A sovereign that accumulated debt while $\nu > 0$, at prices that made the debt look cheap, faces a strictly larger default region the moment $\nu$ turns negative, at the same debt and the same income level. The mechanism is deterministic: the price schedule is a function, a correspondence is unnecessary, and sunspots are inactive, in contrast to \citet{ColeKehoe2000} and \citet{LorenzoniWerning2019}. The fragility is manufactured by the boom itself.
...
\label{eq:cgpanel}
\end{equation}
with standard errors clustered by country and time, delivers a pooled $\beta_{CG}$. Running \eqref{eq:cgpanel} country by country delivers $\theta_i = \theta(\beta_{CG,i})$ by inversion of \eqref{eq:cgmap}, adjusting the mapping for the horizon and time aggregation as derived in Appendix~\ref{app:extensions}. Measurement error in reported forecasts attenuates $\mathrm{Rev}$ coefficients toward zero, and since the same forecast enters $\mathrm{FE}$ and $\mathrm{Rev}$ with opposite signs, classical noise biases $\beta_{CG}$ \emph{negative}. I instrument the revision with its own lag and with other forecasters' revisions. Overlapping fixed-event horizons induce moving-average errors, so the standard errors use the corresponding country-time clustering. The country-level $\theta_i$ then enters the spread test directly: countries with larger measured overreaction should have larger conditional news coefficients.

The payoff of the measurement is that forecasts discipline the quantitative model's belief parameter: $\theta$ comes from forecasts, everything else from \citet{Arellano2008}, and the spread implications are predictions rather than fits.

\section{Quantitative Analysis}\label{sec:quant}

\subsection{Calibration and solution}

I solve the model on the quarterly calibration of \citet{Arellano2008} for Argentina, reproduced in Table~\ref{tab:calibration} with each parameter classified by source. The exercise proceeds in two steps. The first varies only the belief parameter $\theta \in \{0, 0.5, 1\}$ at fixed parameters outside beliefs, asking what a belief distortion in the published individual-forecast range does to a known benchmark. The second step, in Section~\ref{sec:recal}, re-selects $(\beta, \phi)$ with $\theta$ fixed at the baseline value and reports the resulting economy. Appendix~\ref{app:computation} contains the full parameter table (Table~\ref{tab:params}): equation location, units, source, sensitivity range, and the effect of each parameter on the headline numbers. The income process is discretized on a 21-state \citet{Tauchen1986} chain and debt on a 200-point grid. The equilibrium is computed by iterating the value function and the price schedule jointly to a tolerance of $5 \times 10^{-8}$, and the diagnostic kernel is rediscretized exactly as a mean-shifted Gaussian on the same grid (Lemma~\ref{lem:tilt}). A full solution takes seconds, since the belief state adds a single grid dimension to the pricing equation and leaves the government's continuation grid unchanged.
...
The magnitudes are small, single basis points of permanent consumption, for the usual reason: in this class of models default's resource cost is modest and consumption smoothing is cheap. Two amplifications are built into the natural quantitative extension. Long-duration debt adds the dilution externality, which interacts with optimism because booms are exactly when dilution is cheapest (Appendix~\ref{app:extensions} sets up the recursion). Richer default costs of the \citet{MendozaYue2012} type scale every welfare number. The lesson itself is clean: \emph{under one-period debt, a benevolent government, and the computed equilibria at $\theta = 0.5$, a debt ceiling lowers sovereign welfare when the government prices the future correctly and raises it when the government extrapolates}. In this environment fiscal rules are a commitment device against the government's own extrapolation rather than a correction of market mispricing. The state variable the model identifies, the run of recent news, is observable in real time from forecast revisions, so a rule that tightens after sustained upward revisions is the natural state-contingent version of the flat ceiling studied here.

\section{Conclusion}\label{sec:conclusion}

I have taken the workhorse model of sovereign default and replaced one primitive: creditors who overreact to news in the disciplined sense of diagnostic expectations, with the degree of overreaction measured from forecast surveys rather than fitted to prices. The cost was one observable state variable. The return was a set of predictions the rational model rules out exactly. Prices depend on the direction of recent news conditional on fundamentals, and bond returns are predictably poor after good news. Default regions expand when optimism reverses, so defaults cluster after booms on income paths where rational pricing generates near-zero default risk, and spread volatility rises severalfold alongside calmer medians. The predictions survive re-selecting the parameters outside beliefs to restore the observed default frequency, and at that data-consistent calibration the belief distortion accounts for roughly two thirds of defaults. Each prediction comes with an estimating equation and a pre-stated magnitude. The panels that would confirm or refute them are named, and the theory is written so that a zero within-cell news coefficient would falsify it.
...
Three extensions complete the program. Long-duration bonds put diagnostic booms and dilution into the same state variable. Risk-averse lenders let forecast data separate time-varying risk premia from belief distortions. Country-level estimates of $\theta_i$ from the forecasters who price these bonds turn the belief parameter into a measured input. The model stands on that number, which is where a theory of beliefs ought to stand.
...
Write $m_x = \rho x$ and $m_h = \rho h$. Up to factors independent of $x'$,
\[
