...
# The coordination multiplier varies with the state even at fixed primitives.
# This design distribution spans news, composition, effective pessimism, and
# every precision cell satisfying the contraction condition. It is a model
# state distribution, not a sampling distribution.
state_y_grid = collect(0.80:0.025:1.20)
state_mu_grid = [0.20, 0.40, 0.60, 0.80]
state_delta_grid = [0.04, 0.08, 0.12]
multiplier_values = Float64[]
for cell in uniq_cells, y in state_y_grid, mu in state_mu_grid,
        delta in state_delta_grid
    alp = 1 / cell.sigy^2
    bet = 1 / cell.sigx^2
    threshold = tstar(mu, y; delF = delta, alp = alp, bet = bet)
    slope = slope_at(
        threshold, mu, y; delF = delta, alp = alp, bet = bet)
    0 <= slope < 1 || error("a unique-grid multiplier left the contraction region")
    push!(multiplier_values, 1 / (1 - slope))
end
multiplier_quantiles = quantile(
    multiplier_values, [0.10, 0.25, 0.50, 0.75, 0.90])
multiplier_maximum = maximum(multiplier_values)
...
@printf("MULTIPLIER DESIGN DISTRIBUTION (%d states):\n", length(multiplier_values))
@printf("  p10 %.2f, p25 %.2f, median %.2f, p75 %.2f, p90 %.2f, max %.2f\n\n",
        multiplier_quantiles[1], multiplier_quantiles[2],
        multiplier_quantiles[3], multiplier_quantiles[4],
        multiplier_quantiles[5], multiplier_maximum)
...
open(joinpath(OUTDIR, "info_numbers.tex"), "w") do io
    ...
    @printf(io, "\\newcommand{\\MultPten}{%.2f}\n", multiplier_quantiles[1])
    @printf(io, "\\newcommand{\\MultPtwentyfive}{%.2f}\n", multiplier_quantiles[2])
    @printf(io, "\\newcommand{\\MultMedian}{%.2f}\n", multiplier_quantiles[3])
    @printf(io, "\\newcommand{\\MultPseventyfive}{%.2f}\n", multiplier_quantiles[4])
    @printf(io, "\\newcommand{\\MultPninety}{%.2f}\n", multiplier_quantiles[5])
    @printf(io, "\\newcommand{\\MultMaximum}{%.2f}\n", multiplier_maximum)
    @printf(io, "\\newcommand{\\MultStateCount}{%d}\n", length(multiplier_values))
end
