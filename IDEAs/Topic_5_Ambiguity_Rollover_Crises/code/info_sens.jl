# Information-scenario sensitivity for the ambiguity rollover model.
#
# Referee demand (Section 5.3 / 6.2): the information block (sigx, sigy) is a
# SCENARIO, not an estimated object. So we must (a) verify the uniqueness
# condition ALP/sqrt(BET) < sqrt(2*pi) across the whole plausible precision
# region rather than at the calibrated point, and (b) report how the headline
# distrust premium moves across that region, so the paper states an honest band.
#
# This script does NOT modify solve_model.jl. The primitives below (constants,
# ncdf/npdf/nquant, pbar, tstar, crisisP, spread_bp) are copied VERBATIM from
# solve_model.jl so the reproduced baseline is bit-for-bit the published one.
# solve_model.jl runs main() unguarded, so we copy rather than include it.
#
...
# consequence, reported honestly below via \SCalmLo/\SCalmHi, is that the calm
# spread itself is scenario-dependent (the threshold and crisisP both move with
# precision), so the calm anchor is not invariant across the band.
#
...
# =============================================================== 2. GRID SWEEP
# Information-scenario grid. Precision spans well over a factor of 2 in each
...
println("FRONTIER:")
@printf("  at sigy = 0.25, uniqueness fails once sigx >= %.4f (private noise ceiling)\n", sigx_front)
@printf("  at sigx = 0.15, uniqueness fails once sigy <= %.4f (public noise floor)\n", sigy_front)
...
open(joinpath(OUTDIR, "info_numbers.tex"), "w") do io
    println(io, "% Information-scenario sensitivity macros (generated by code/info_sens.jl).")
